...
-----------------------------------------------
@description:
Use a hashset to record all nums appeared in the first list,
then check if there are nums in the second list have appeared in the hashset.
...
class Solution:
    def intersection(self, nums1, nums2):
        ...
        hash_set = {}
        for i in nums1:
            hash_set[i] = hash_set[i] + 1 if i in hash_set else 1
        for j in nums2:
            if j in hash_set and hash_set[j] > 0:
                res.append(j)
                hash_set[j] -= 1
